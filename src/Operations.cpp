...
stack<Frame> Operations::threads;
Frame_stack Operations::frame_stack = NULL;
...
uint32_t Operations::get_n_bytes_value(uint8_t n, unsigned char** pc)
{
  uint32_t value = **pc;
  *pc +=1;
...
void Operations::set_frame(Frame newRef) {
	frame = newRef;
}
...
void Operations::set_frame_stack(Frame_stack newFS) {
	frame_stack = newFS;
}
...
void Operations::iload () {
	...
	Typed_element aux = frame.local_variables->get(int(index));
...
{
	Typed_element aux = frame.local_variables->get(index);
	frame.operand_stack->push(long(aux.value.l));
}
...
void Operations::lstore() {
   ...
   if (isWide) {
	...
	if(frame.operand_stack->top_type() == TYPE_LONG) {
		...
		frame.local_variables->insert_typed_element(index, aux);
	}
	else
		printf("Operando no topo != TYPE_LONG\n");
}
...
void Operations::istore_0() {
	if(frame.operand_stack->top_type() == TYPE_INT) {
		Typed_element aux = frame.operand_stack->pop_typed_element();
		frame.local_variables->insert_typed_element(0, aux);
	}
	else
		printf("Operando no topo != TYPE_INT\n");
}
...
void Operations::istore_2() {
	if(frame.operand_stack->top_type() == TYPE_INT) {
		Typed_element aux = frame.operand_stack->pop_typed_element();
		frame.local_variables->insert_typed_element(2, aux);
	}
	else
		printf("Operando no topo != TYPE_INT\n");
}
...
void Operations::istore_3() {
	if(frame.operand_stack->top_type() == TYPE_INT) {
		Typed_element aux = frame.operand_stack->pop_typed_element();
		frame.local_variables->insert_typed_element(3, aux);
	}
	else
		printf("Operando no topo != TYPE_INT\n");
}
...
void Operations::faload() {
	...
	LocalVariables *arrayref = (LocalVariables *) frame.operand_stack->pop().pi;
	if(arrayref == NULL){
	}
...
void Operations::lastore() {
	Element valor = frame.operand_stack->pop();
	Element indice = frame.operand_stack->pop();
	LocalVariables *vetor = (LocalVariables *) frame.operand_stack->pop().pi;
...
void Operations::iinc(){
	...
	Typed_element aux = frame.local_variables->get(var);
...
void Operations::funcret() {
	if (isWide){
		frame.pc = (unsigned char *) frame.local_variables->get(get_n_bytes_value(2, frame.pc)).value.pi;
		isWide = false;
	}
	else
		frame.pc = (unsigned char *) frame.local_variables->get(get_n_bytes_value(1, frame.pc)).value.pi;
}
...
ClasseEstatica* Operations::getStaticClassThatHasField(ClasseEstatica* base, string field_name) 
{
...
void Operations::getstatic()
{
    ...
    string class_name = dereferenceIndex(frame.cp_vector, cp_Element.info[0].u2);
    ...
    string name = dereferenceIndex(frame.cp_vector, name_and_type_Element.info[0].u2);
    string descriptor = dereferenceIndex(frame.cp_vector, name_and_type_Element.info[1].u2);
...
void Operations::getfield() {
    ...
    ClasseInstancia *ci = (ClasseInstancia *) frame.operand_stack->pop().pi;
    ...
    Typed_element ret = ci->getField(dereferenceIndex(frame.cp_vector, index));
...
void Operations::putfield() {
    ...
    if (dereferenceIndex(frame.cp_vector, frame.m.name_index) == "<init>") {
        ci->setFinals(dereferenceIndex(frame.cp_vector, index), value);
    } else {
        ci->setField(dereferenceIndex(frame.cp_vector, index), value);
    }
}
...
void Operations::invokespecial() {
    ...
    string classe = dereferenceIndex(frame.cp_vector, cp_Element.info[0].u2);
    ...
    string name = dereferenceIndex(frame.cp_vector, metodo.info[0].u2);
    string desc = dereferenceIndex(frame.cp_vector, metodo.info[1].u2);
...
void Operations::func_new(){
    uint16_t indexbyte = get_n_bytes_value(2, frame.pc);
    string classe = dereferenceIndex(frame.cp_vector, indexbyte);
    ...
    frame.operand_stack->push((int*)aux->getInstance());
}
...
void Operations::arraylength()
{
    ...
    arrayref = (LocalVariables *) frame.operand_stack->pop().pi;
    if (arrayref == nullptr)
        throw runtime_error("Null pointer");

    frame.operand_stack->push(arrayreframe.getMax());
}

void Operations::athrow()
{
    int type = frame.operand_stack->top_type();
    Element_u value = frame.operand_stack->pop();
